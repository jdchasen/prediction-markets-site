...
This is a critical detail that many new traders overlook. Weather contracts on Kalshi settle based on **official weather station data** -- specifically, the recorded daily high temperature as reported by NOAA-affiliated stations. Settlement doesn't happen at midnight on the observation day. Instead, contracts typically settle **the next morning** after the observation day, once official data has been published and verified.
...
The single biggest advantage in weather markets is that **professional-grade forecast data is freely available**. Unlike sports betting or political markets, you don't need insider information or proprietary models. The National Weather Service publishes detailed forecasts, and multiple weather APIs provide granular temperature predictions.
...
Markets for events **1 to 3 days in the future** offer the best risk-reward balance. Forecasts at this range are good enough to give you a directional edge, but uncertain enough that the market hasn't fully converged to the correct price. You'll find the widest mispricings here, especially when a forecast model update shifts the expected high by a degree or two and the market hasn't caught up yet.

### Same-Day Markets: A Trap for the Unwary

This is the single most important practical warning I can give: **don't trade same-day weather markets in the afternoon**. By mid-afternoon, the daily high temperature in most cities has already been observed. The outcome is effectively known, even though the contract hasn't settled yet. You might look at a contract and see what appears to be an edge, but the high for the day has already been recorded at the weather station. You're not predicting anything -- you're just racing against settlement with stale data.
...
The fix is simple: **require a minimum time buffer before settlement**. I now skip any market that settles within the next 18 hours. That ensures you're always trading into genuine uncertainty, not into an outcome that has already occurred.

### Watch for Forecast Model Updates

Weather forecast models (GFS, ECMWF, NAM) run on regular cycles -- typically every 6 or 12 hours. When a new model run comes in and shifts the forecast high by even 1-2 degrees, it can create a temporary mispricing before the Kalshi market adjusts. If you're monitoring forecast updates programmatically, you can be among the first to act on these shifts.
...
1. **Quantifiable inputs**: Temperature forecasts are numerical. You don't need to interpret qualitative information or sentiment.
...
- **Summer markets in desert cities** (Phoenix, Las Vegas) are often too easy to forecast. The high is 110 degrees F for weeks at a time, and the contracts are priced near $0.95 or $0.05. There's no edge when the outcome is obvious.
